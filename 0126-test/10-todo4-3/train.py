...
print(f"lambda_update_frequency: {config['Q']}")
print("--------------------")
...
with strategy.scope():
    ...
    model = model_class(tau=config['tau'], rho=config['rho'], max_multiplier=config['max_multiplier'], lambda_update_frequency=config['Q'], fcd_mode=config['fcd_mode'], dense_stats=dense_stats)
