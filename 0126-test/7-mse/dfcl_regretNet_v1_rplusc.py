...
class EcomDFCL_regretNet_rplusc(tf.keras.Model):
    ...
    def factual_mse_constraint_old(self, predictions, labels):
        """
    ...
    def factual_mse_constraint(self, predictions, labels):
        ...
        只在真实发生的 treatment 上计算 MSE（通过 one-hot mask 实现）：
            ...
            mse_r = sum_i ( (r_hat[i, t_i] - y_r[i])^2 ) / N_valid
            mse_c = sum_i ( (c_hat[i, t_i] - y_c[i])^2 ) / N_valid
        ...
        # valid 样本：col_idx >= 0
        valid = col_idx >= 0                                # [B] bool
        safe_col_idx = tf.where(valid, col_idx, tf.zeros_like(col_idx))  # 把无效的先置 0 防止 one_hot 报错

        # one-hot mask: [B, T]，每行只有真实 treatment 列为 1（无效样本行会先被置 0）
        mask = tf.one_hot(safe_col_idx, depth=tf.shape(r_hat)[1], dtype=tf.float32)  # [B, T]
        # 把无效样本整行 mask 清零
        mask = mask * tf.cast(tf.expand_dims(valid, 1), tf.float32)                  # [B, T]

        # ====== 3) 计算所有 treatment 的 squared error，再用 mask 选出 factual ======
        # 把 label 扩成 [B, 1] 以便 broadcast 到 [B, T]
        ...
        # 只保留 factual treatment 的误差（每行最多一个元素非 0）
        se_r_factual = tf.reduce_sum(se_r_all * mask, axis=1)  # [B]
        se_c_factual = tf.reduce_sum(se_c_all * mask, axis=1)  # [B]

        # ====== 4) masked mean（按有效样本数归一） ======
        denom = tf.reduce_sum(tf.cast(valid, tf.float32))  # N_valid
        denom = tf.maximum(denom, 1.0)                     # 防止全无效时除 0
