...
class EcomDFCL_regretNet_rplusc(tf.keras.Model):
    ...
    def _get_current_lr(self):
        """返回当前 step 的 learning rate（兼容常数 lr / Variable / schedule）"""
        ...
        if isinstance(lr, tf.keras.optimizers.schedules.LearningRateSchedule):
            ...
            # 常数 / tf.Variable / tensor
            lr_t = lr

        lr_t = tf.convert_to_tensor(lr_t)
        return tf.cast(lr_t, tf.float32)
    ...
    def train_step(self, data):
        ...
        def update_mu():
            # ===== 修改：dual 更新步长用当前 learning rate（对齐 primal/dual 同步尺度的尝试）=====
            dual_step = self._get_current_lr() * self.dual_lr_scale

            # μ_{k+1} = clip( μ_k + η_dual * g(w) )
            new_mu = self.mu + dual_step * tf.stop_gradient(prediction_loss)
        ...
        tf.summary.scalar("optim/current_lr", current_lr, step=step)
        tf.summary.scalar("lagrangian/dual_step", current_lr * self.dual_lr_scale, step=step)
