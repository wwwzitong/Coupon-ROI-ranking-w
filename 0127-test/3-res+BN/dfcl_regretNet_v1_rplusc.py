...
class EcomDFCL_regretNet_rplusc(tf.keras.Model):
    ...
    def __init__(self, rho=0.1, dense_stats=None, fcd_mode='log1p', lambda_update_frequency=20, max_multiplier=1.0, tau=1.0, **kwargs):
        ...
        # 网络结构（升级版）：Residual + LayerNorm，深一点但更稳
        self.user_tower = tf.keras.Sequential([
            # Stem
            tf.keras.layers.Dense(512, activation=None, kernel_initializer='glorot_normal'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Activation("swish"),
            tf.keras.layers.Dropout(0.3),

            # 深化：残差块堆叠（你可以从 2 个 block 开始，稳定后再加）
            ResidualMLPBlock(512, dropout=0.3, activation="swish", name="res512_1"),
            ResidualMLPBlock(512, dropout=0.3, activation="swish", name="res512_2"),

            # 下采样到 256
            tf.keras.layers.Dense(256, activation=None, kernel_initializer='glorot_normal'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Activation("swish"),
            tf.keras.layers.Dropout(0.3),

            ResidualMLPBlock(256, dropout=0.3, activation="swish", name="res256_1"),

            # 输出维度到 128（作为 shared representation）
            tf.keras.layers.Dense(128, activation=None, kernel_initializer='glorot_normal'),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Activation("swish"),
        ], name='user_tower')


        # 显式构建 user_tower（保留你原来的逻辑）
        ...
        # task towers（升级版）：head 稍微加深 + 残差，让不同 treatment/target 的映射更有表达力
        self.task_towers = {}
        for target in self.targets:
            for treatment in self.treatment_order:
                ...
                self.task_towers[name] = tf.keras.Sequential([
                    ...
                    ResidualMLPBlock(128, dropout=0.3, activation="swish", name=f"{name}_res128"),

                    tf.keras.layers.Dense(64, activation=None, kernel_initializer='glorot_normal'),
                    tf.keras.layers.BatchNormalization(),
                    tf.keras.layers.Activation("swish"),
                    tf.keras.layers.Dropout(0.3),
