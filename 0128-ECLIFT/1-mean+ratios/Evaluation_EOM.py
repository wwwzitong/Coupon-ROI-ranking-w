...
config = {
    'eval_data': '/root/autodl-tmp/data/ECLIFT_test.csv',
    'batch_size': 1024*16,
    ...
    # ---- NEW: EOM ----
    'eom_save_path': "result/result_eom.json",
    # 若为 None：会在可达的 per-capita cost 范围内自动均匀取 num_eom_budgets 个预算点
    'eom_budgets': None,          # e.g. [0.1, 0.2, 0.3, ...]（per-capita cost）
    'num_eom_budgets': 6,
    'eom_eps': 1e-6,
    'eom_max_iter': 60,
}
...
model_paths_DFCL = [
    ...
    # "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs4096_lr1e-3_clip=5e3_max=1_tau=1.0",
    # "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs4096_lr1e-4_clip=5e3_max=1_tau=1.0",
    # "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs4096_lr5e-4_clip=5e3_max=1_tau=1.0",

]
model_paths_else = [
    "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs8192_lr5e-4_clip=5e3_max=1_tau=1.0",
    "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs8192_lr1e-3_clip=5e3_max=1_tau=1.0",
    "./model/EcomDFCL_regretNet_rplusc_wce_mean+ratios_bs8192_lr2e-3_clip=5e3_max=1_tau=1.0",
]
...
# ====================（2）新增：EOM 计算函数（Policy Evaluation Based on EOM）====================
def _estimate_propensity(df, treatment_col='treatment'):
    """RCT/随机实验数据：用经验频率估计 p(t)（也可替换成已知分流概率）。"""
    vc = df[treatment_col].value_counts(normalize=True)
    return {int(k): float(v) for k, v in vc.items()}

def _action_from_lambda(r_hat, c_hat, lam):
    """z_i = argmax_j (r_hat_ij - lam * c_hat_ij)"""
    return np.argmax(r_hat - lam * c_hat, axis=1).astype(np.int32)

def _eom_ipw_percap(df, action, propensity, reward_col='paid', cost_col='cost', treatment_col='treatment'):
    """
    EOM: 只用“事实一致”的样本（t_i == action_i），并做 1/p(t_i) 逆倾向加权，
    输出 per-capita revenue/cost（按论文定义除以 N）。
    """
    t = df[treatment_col].astype(int).to_numpy()
    r = df[reward_col].to_numpy()
    c = df[cost_col].to_numpy()

    p = np.vectorize(lambda x: propensity.get(int(x), 0.0))(t)
    p = np.clip(p, 1e-12, None)  # 防止除零

    match = (t == action)
    w = match.astype(np.float64) / p

    N = len(df)
    r_bar = float(np.sum(w * r) / N)
    c_bar = float(np.sum(w * c) / N)
    return r_bar, c_bar

def _lambda_upper_bound(r_hat, c_hat):
    """论文 Appendix C：lambda_max = max_{i,j} (r_hat_ij / c_hat_ij)（c_hat=0 的点忽略）"""
    denom = np.where(c_hat > 0, c_hat, np.nan)
    ratio = r_hat / denom
    lam_max = np.nanmax(ratio)
    if not np.isfinite(lam_max):
        lam_max = 1.0
    return float(lam_max)

def _solve_lambda_for_budget(df, r_hat, c_hat, budget_per_capita, propensity,
                            eps=1e-6, max_iter=60, reward_col='paid', cost_col='cost', treatment_col='treatment'):
    """
    二分搜索 lambda，使得 EOM 估计的 per-capita cost 接近 budget_per_capita。
    cost(lambda) 通常随 lambda 增大而不增（更偏好低成本动作）。
    """
    lam_lo = 0.0
    lam_hi = _lambda_upper_bound(r_hat, c_hat)

    # 先算端点
    a_lo = _action_from_lambda(r_hat, c_hat, lam_lo)
    _, c_lo = _eom_ipw_percap(df, a_lo, propensity, reward_col, cost_col, treatment_col)

    a_hi = _action_from_lambda(r_hat, c_hat, lam_hi)
    _, c_hi = _eom_ipw_percap(df, a_hi, propensity, reward_col, cost_col, treatment_col)

    # 目标预算超出可达范围：直接截断到最近端点
    if budget_per_capita >= c_lo:
        return lam_lo
    if budget_per_capita <= c_hi:
        return lam_hi

    # 二分
    for _ in range(max_iter):
        lam_mid = 0.5 * (lam_lo + lam_hi)
        a_mid = _action_from_lambda(r_hat, c_hat, lam_mid)
        _, c_mid = _eom_ipw_percap(df, a_mid, propensity, reward_col, cost_col, treatment_col)

        if abs(c_mid - budget_per_capita) <= eps:
            return float(lam_mid)

        # 如果当前成本仍高于预算，需要更大 lambda 来压成本
        if c_mid > budget_per_capita:
            lam_lo = lam_mid
        else:
            lam_hi = lam_mid

    return float(0.5 * (lam_lo + lam_hi))


# ====================（A）修改：calculate_and_save_eom 增加 baseline & incremental_reward ====================
def calculate_and_save_eom(
    df,
    pred_reward_cols=('ctrl_paid', 'treat_paid'),
    pred_cost_cols=('ctrl_cost', 'treat_cost'),
    reward_col='paid',
    cost_col='cost',
    ...
    # 组装预测矩阵（按动作 j=0/1）
    r_hat = np.stack([df[pred_reward_cols[0]].to_numpy(), df[pred_reward_cols[1]].to_numpy()], axis=1).astype(np.float64)
    c_hat = np.stack([df[pred_cost_cols[0]].to_numpy(),  df[pred_cost_cols[1]].to_numpy()],  axis=1).astype(np.float64)

    propensity = _estimate_propensity(df, treatment_col=treatment_col)

    # ---- NEW: baseline policy（全选 control=0）用于归一化，形成类似论文图的 Incremental Reward ----
    baseline_action = np.zeros(len(df), dtype=np.int32)
    baseline_r_bar, baseline_c_bar = _eom_ipw_percap(
        df, baseline_action, propensity,
        reward_col=reward_col, cost_col=cost_col, treatment_col=treatment_col
    )
    baseline_r_bar = float(baseline_r_bar)

    # 自动生成预算点：在可达 cost 范围内均匀取点（从低到高）
    if budgets is None:
        lam0 = 0.0
        lamM = _lambda_upper_bound(r_hat, c_hat)
        a0 = _action_from_lambda(r_hat, c_hat, lam0)
        _, c0 = _eom_ipw_percap(df, a0, propensity, reward_col, cost_col, treatment_col)
        aM = _action_from_lambda(r_hat, c_hat, lamM)
        _, cM = _eom_ipw_percap(df, aM, propensity, reward_col, cost_col, treatment_col)

        c_min, c_max = (min(c0, cM), max(c0, cM))
        if num_budgets <= 1:
            budgets = [c_max]
        else:
            budgets = np.linspace(c_min, c_max, num_budgets).tolist()

    results = []
    for b in budgets:
        lam = _solve_lambda_for_budget(
            df, r_hat, c_hat,
            budget_per_capita=float(b),
        ...
        action = _action_from_lambda(r_hat, c_hat, lam)
        r_bar, c_bar = _eom_ipw_percap(df, action, propensity, reward_col, cost_col, treatment_col)

        inc_reward = (float(r_bar) / baseline_r_bar) if baseline_r_bar != 0 else float("nan")

        results.append({
            "budget_per_capita": float(b),
            "lambda": float(lam),
            "per_capita_reward": float(r_bar),
            "per_capita_cost": float(c_bar),
            # ---- NEW ----
            "incremental_reward": float(inc_reward),
        })

    # 保存到 JSON（按 model_path 索引）
    try:
        with open(eom_save_path, "r", encoding="utf-8") as f:
            all_results = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        all_results = {}
    ...
    all_results[model_path] = {
        ...
        "baseline": {
            "policy": "all_control",
            "per_capita_reward": float(baseline_r_bar),
            "per_capita_cost": float(baseline_c_bar),
        },
        "results": results,
    }

    with open(eom_save_path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=4, ensure_ascii=False)

    return results


# ====================（B）新增：从 result_eom.json 画 “Budget- Incremental Reward” 多模型对比图 ====================
def plot_eom_from_json(
    json_path: str,
    plot_path: str,
    model_names: Optional[List[str]] = None,
    y_key: str = "incremental_reward",   # 画论文那种就用 incremental_reward；想看原值就用 per_capita_reward
    title: str = "EOM",
):
    try:
    ...
    if model_names:
        ...
        not_found = set(model_names) - set(results_to_plot.keys())
        if not_found:
    ...
    for model_name, data in results_to_plot.items():
        if "results" not in data or not data["results"]:
            print(f"模型 '{model_name}' 无 EOM results，跳过。")
            continue

        rows = sorted(data["results"], key=lambda x: x["budget_per_capita"])
        y = [float(r.get(y_key, np.nan)) for r in rows]

        # x 轴用 1..K（更像论文图）；需要真实 budget 就改成 budget_per_capita
        x = list(range(1, len(rows) + 1))

        plt.plot(x, y, marker='o', linestyle='-', linewidth=2, label=model_name)

    plt.xlabel("	                 Budget", fontsize=13)
    plt.ylabel("Incremental Reward" if y_key == "incremental_reward" else y_key, fontsize=13)
...
for model_path in model_paths_DFCL: 
    ...
    with tee_output(f"{model_path}/eval.log", mode="a", encoding="utf-8"):
        ...
        # ---- NEW: 计算 EOM（per-capita reward under per-capita budgets）----
        print("\n正在计算 EOM 指标（Expected Outcome Metric）...")
        eom_results = calculate_and_save_eom(
            df=eval_df,
            pred_reward_cols=('ctrl_paid', 'treat_paid'),
            pred_cost_cols=('ctrl_cost', 'treat_cost'),
            reward_col='paid',
            cost_col='cost',
            treatment_col='treatment',
            budgets=config.get('eom_budgets', None),
            num_budgets=config.get('num_eom_budgets', 6),
            eps=config.get('eom_eps', 1e-6),
            max_iter=config.get('eom_max_iter', 60),
            eom_save_path=config.get('eom_save_path', "result/result_eom.json"),
            model_path=model_path,
        )

        # 打印成表格（按预算从小到大）
        eom_results_sorted = sorted(eom_results, key=lambda x: x["budget_per_capita"])
        print("\nEOM 结果（per-capita）:")
        print("  budget_per_capita\tlambda\t\tper_capita_cost\tper_capita_reward")
        for row in eom_results_sorted:
            print(f"  {row['budget_per_capita']:.6f}\t\t{row['lambda']:.6f}\t\t{row['per_capita_cost']:.6f}\t\t{row['per_capita_reward']:.6f}")

        print(f"\nEOM 已保存至: {config.get('eom_save_path', 'result/result_eom.json')}")
...
eom_json_path = config.get("eom_save_path", "result/result_eom.json")
eom_plot_path = f"result/eom_curves_{current_time}.png"

plot_eom_from_json(
    json_path=eom_json_path,
    plot_path=eom_plot_path,
    model_names=model_paths_DFCL + model_paths_else,
    y_key="incremental_reward",
    title="EOM (Marketing data)"
)