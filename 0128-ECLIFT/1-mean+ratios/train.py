...
# ---- GPU 初始化：建议尽早执行（避免一次性占满显存）----
gpus = tf.config.list_physical_devices('GPU')
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        print(f"[GPU] Visible GPUs: {gpus}")
    except RuntimeError as e:
        # set_memory_growth 必须在 GPU 初始化前设置；如果这里报错，说明 TF 已经初始化过
        print(f"[GPU] set_memory_growth failed: {e}")
else:
...
config = {
    ...
    'last_model_path': '',
    'train_data': '/root/autodl-tmp/data/ECLIFT_train.csv', 
    'val_data': '/root/autodl-tmp/data/ECLIFT_val.csv',
    'batch_size': 4096,
    'num_epochs': 50,
...
print(f'策略: {type(strategy).__name__}, 副本数: {strategy.num_replicas_in_sync}')
global_batch_size = config['batch_size'] * strategy.num_replicas_in_sync
# In[10]:
