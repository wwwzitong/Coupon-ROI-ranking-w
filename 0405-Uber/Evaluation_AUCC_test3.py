...
"""
Non-RCT AUCC evaluation.

保留两套原始绘图链路，但把两套 AUCC 评估都替换成 Non-RCT corrected 版本：

1. strict_aucc_algorithm2
   - 对应你原来的 strict_aucc_algorithm2
   - 逐点 / 排序位置取点，不做公司版 bin 平滑
   - 保存 JSON: result/result_aucc_strict_nonrct.json
   - 最终绘图: result/aucc_curves_strict_nonrct_<timestamp>.pdf

2. calculate_and_save_aucc
   - 对应你原来的 calculate_and_save_aucc
   - 分桶平滑版本
   - 保存 JSON: result/result_aucc_binned_nonrct.json
   - 最终绘图: result/aucc_curves_binned_nonrct_<timestamp>.pdf

get_aucc_plot 的两张图已去掉。
"""
...
config = {
    ...
    "aucc_binned_save_path": "result/result_aucc_binned_nonrct.json",
}
...
aucc_binned_save_path = config["aucc_binned_save_path"]
...
def _compute_snips_prefix_curves(
    df: pd.DataFrame,
    reward_col: str,
    cost_col: str,
    treatment_col: str,
    ...
    """
    按 uplift_col 排序后，计算 top-k SNIPS corrected 曲线。

    返回：
    - df_sorted
    - selected_fraction
    - delta_gain: corrected incremental reward curve
    - delta_cost: corrected incremental cost curve, C1-C0
    - policy_cost: treatment-side policy cost, normally monotone
    - w_t, w_c: IPS weights
    """
    ...
    return df_sorted, selected_fraction, delta_gain, delta_cost, policy_cost, w_t, w_c


def _make_plot_curve(
    selected_fraction: np.ndarray,
    delta_gain: np.ndarray,
    ...
    mode:
        'strict': 逐点曲线；如果 n_bins=None，不做平滑。为了文件大小，可设 n_bins=1000。
        'binned': 分桶平滑曲线；默认按排序位置取 n_bins 个点。

    x_axis:
        'policy_cost': 推荐。横轴为 treatment-side policy cost，通常单调。
        'fraction': 横轴为 selected fraction。
        'incremental_cost': 横轴为 corrected incremental cost，可能非单调，不推荐默认使用。
    """
    n = len(delta_gain)
    ...
    if mode == "strict":
        ...
        raise ValueError("mode 只能是 'strict' 或 'binned'。")
    ...
    if np.isclose(y_den, 0.0):
        print("[AUCC] final incremental gain 接近 0，改用 max(abs(gain)) 做绘图尺度。")
    ...
    if np.any(np.diff(x) < -1e-12):
        print(
            f"[AUCC] 警告：x_axis={x_axis_used} 横轴非单调，"
...
def strict_aucc_algorithm2(
    ...
    max_points_for_json: Optional[int] = None,
    save_debug_csv_path: Optional[str] = None,
) -> float:
    """
    矫正后的 strict_aucc_algorithm2。

    对应你原始 strict_aucc_algorithm2：
    - 仍然按 uplift_col 降序排序；
    - 仍然先保存 JSON，然后由 plot_aucc_from_json 统一绘图；
    - 差别是 delta_gain / delta_cost 使用 IPS/SNIPS 校正；
    - 默认不分桶；如数据太大，可设置 max_points_for_json=1000 降低 JSON 大小。
    """
    ...
    df_sorted, selected_fraction, delta_gain, delta_cost, policy_cost, w_t, w_c = _compute_snips_prefix_curves(
        df=df_filtered,
    ...
    x, y, meta = _make_plot_curve(
        selected_fraction=selected_fraction,
        delta_gain=delta_gain,
        delta_cost=delta_cost,
        policy_cost=policy_cost,
        mode="strict",
        n_bins=max_points_for_json,
        x_axis=x_axis,
    )
    ...
    if save_debug_csv_path:
        ...
        print(f"[strict_aucc_algorithm2 corrected] debug 已保存: {save_debug_csv_path}")
    ...
    _save_curve_json(
        ...
        x=x,
        y=y,
        method="strict_aucc_algorithm2_nonrct_ips_snips",
        meta=meta,
        propensity_col=propensity_col,
        eps=eps,
    )
    ...
    print(
        f"[strict_aucc_algorithm2 corrected] 保存成功: {save_path}, "
        f"score={aucc_score:.6f}, "
        f"x_axis={meta['x_axis']}, "
...
def calculate_and_save_aucc(
    ...
    对应你原始公司版 calculate_and_save_aucc：
    - 仍然是“更平滑”的分桶版；
    - 仍然先保存 JSON，然后由 plot_aucc_from_json 统一绘图；
    - 差别是 delta_gain / delta_cost 使用 IPS/SNIPS 校正；
    - 分桶方式改为按排序位置取点，而不是按 delta_cost pd.cut。
      这样避免 non-RCT 校正后 incremental cost 非单调导致曲线回折/横轴挤压。
    """
    if save_path is None:
        save_path = aucc_binned_save_path
    if model_key is None:
        model_key = "unknown_model"
    ...
    if save_debug_csv_path:
        ...
        debug_df["selected_fraction"] = selected_fraction
        debug_df.to_csv(save_debug_csv_path, index=False, encoding="utf-8-sig")
        print(f"[calculate_and_save_aucc corrected] debug 已保存: {save_debug_csv_path}")
    ...
    _save_curve_json(
        ...
        x=x,
        y=y,
        method="calculate_and_save_aucc_nonrct_ips_snips_binned",
        meta=meta,
        propensity_col=propensity_col,
        eps=eps,
    )
    ...
    print(
        f"[calculate_and_save_aucc corrected] 保存成功: {save_path}, "
        f"score={aucc_score:.6f}, "
        f"x_axis={meta['x_axis']}, "
        f"x_range=({np.nanmin(x):.4f}, {np.nanmax(x):.4f}), "
        f"y_range=({np.nanmin(y):.4f}, {np.nanmax(y):.4f}), "
...
for model_path in model_paths_DFCL:
    ...
    with tee_output(f"{model_path}/eval.log", mode="a", encoding="utf-8"):
        ...
        aucc_score_binned = calculate_and_save_aucc(
            df=eval_df,
            reward_col="paid",
            cost_col="cost",
            treatment_col="treatment",
            ...
            feature_prefix="x__",
            x_axis="policy_cost",
            save_debug_csv_path=f"{model_path}/result/aucc_binned_nonrct_debug.csv",
        )
        print(f"模型 {model_path} corrected binned AUCC 分数为: {aucc_score_binned:.6f}")
...
binned_output_image_path = f"result/aucc_curves_binned_nonrct_{current_time}.pdf"
plot_aucc_from_json(
    aucc_binned_save_path,
    binned_output_image_path,
    model_names=model_paths_DFCL + model_paths_else,
    model_name_map=model_name_map,
)
