...
class EcomDFCL_regretNet_rc(tf.keras.Model):
    ...
    只参考形式，但还没有完全还原。
    """
    def __init__(self, rho=0.1, lambda_update_frequency=20, **kwargs):
        ...
        # 拉格朗日乘子，对应 conversion_loss 和 visit_loss 两个约束
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # g(w) 是约束函数: [conversion_loss, visit_loss] (我们希望它们为0)
            # λ 是拉格朗日乘子: self.lagrange_multipliers
            
            constraint_violations = tf.stack([conversion_loss, visit_loss])
        ...
        tf.summary.scalar("lagrangian/lambda_conversion", self.lagrange_multipliers[0], step=step)
        tf.summary.scalar("lagrangian/lambda_visit", self.lagrange_multipliers[1], step=step)
