...
class EcomDFCL_regretNet_tau(tf.keras.Model):
    ...
    只参考形式，但还没有完全还原。
    """
    def __init__(self, rho=0.01, lambda_update_frequency=30, **kwargs):
    ...
    def decision_policy_learning_loss(self, predictions, labels): 
        ...
        for ratio in self.ratios:
            ...
            ratio_target = tf.reshape(labels['conversion'] - ratio * labels['visit'], [-1, 1])
    ...
    def tau_LeakyReLU_decision_loss(self, predictions, labels):
        ...
        其中 r 是 conversion，c 是 visit，mean_i 是在批次上的平均值。
        ...
        decision_loss_sum = tf.constant(0.0, dtype=tf.float32)
        uplift_r = pred_dict["conversion_treatment_1"] - pred_dict["conversion_treatment_0"]
        uplift_c = pred_dict["visit_treatment_1"] - pred_dict["visit_treatment_0"]
    ...
    def train_step(self, data):
        ...
        - 约束：conversion_loss 和 visit_loss 趋近于0
        """
        ...
        with tf.GradientTape() as tape:
            ...
            # g(w) 是约束函数: prediction_loss = conversion_loss + visit_loss (我们希望它为0)
            # μ 是拉格朗日乘子: self.mu
            prediction_loss = conversion_loss + visit_loss
        ...
        def update_mu():
            ...
            # 将 mu 限制在 [0, 1] 范围内
            self.mu.assign(tf.clip_by_value(new_mu, 0.0, 1.0))
