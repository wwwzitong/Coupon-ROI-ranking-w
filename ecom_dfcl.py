...
#         shared_output = self.user_tower(concat_input, training=training)
    ...
    def decision_entropy_regularized_loss(self, predictions, labels): # 5.3 引入温度参数τ和最大熵正则化，使得决策损失更加平滑且可微
        ...
        tau = self.tau if hasattr(self, 'tau') else 2.5
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # --- 进阶监控 3: 主动错误检测 ---
            checked_predictions = {
                name: tf.debugging.check_numerics(pred, f"NaN/Inf in prediction: {name}")
        ...
        for target in self.targets:
            # treatment_order = [1, 0]
            uplift_pred = checked_predictions[f'{target}_treatment_{self.treatment_order[0]}'] - \
                          checked_predictions[f'{target}_treatment_{self.treatment_order[1]}']
            self._add_summaries(f"uplift/{target}_uplift_pred", uplift_pred, step=step)
