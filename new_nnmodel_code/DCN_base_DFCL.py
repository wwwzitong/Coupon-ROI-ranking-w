...
class CrossLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.w = self.add_weight(
            ...
            trainable=True, 
            name="cross_weight"  # <--- 加上这个
        )
        self.b = self.add_weight(
            shape=(input_dim,), 
            initializer="zeros", 
            trainable=True, 
            name="cross_bias"    # <--- 加上这个
        )
        super(CrossLayer, self).build(input_shape)

    def call(self, inputs):
        # 显式特征交叉: x_0 * (x * w) + b + x
        x_w = tf.tensordot(inputs, self.w, axes=1)
        x_w = tf.expand_dims(x_w, -1)
        cross = inputs * x_w + self.b
        return cross + inputs


class DCN_base_DFCL(tf.keras.Model): # std+2pos
    """
...
#         self.ratios = [0.1, 0.5, 1.0]  #先用少量测试
        ...
        for target in self.targets:
            ...
            self.task_towers[name] = tf.keras.Sequential([
                ...
                tf.keras.layers.LayerNormalization(), # 加上 LN 训练更稳
