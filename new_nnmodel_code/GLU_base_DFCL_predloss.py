...
class GLU_base_DFCL(tf.keras.Model): # std+2pos
    ...
    def __init__(self, transform_params_path='dense_transform_params.json', **kwargs):
        ...
        # prediction loss 的残差分布假设（TODO 3）
        self.pred_loss_dist = kwargs.pop('pred_loss_dist', 'bernoulli')
        self.t_df = float(kwargs.pop('t_df', 3.0))  # Student-t 的自由度
...
#         self.ratios = [0.1, 0.5, 1.0]  #先用少量测试
        ...
        # NLL loss 的可训练参数（每个 target 一组）
        # - normal / laplace / t: scale 参数（softplus 保证正值）
        # - beta: (a,b) 参数（softplus+1.1 保证 >1）
        # - gamma: shape(a) + scale
        self._nll_params = {}
        for _t in self.targets:
            self._nll_params[_t] = {
                'log_scale': tf.Variable(0.0, trainable=True, name=f'{_t}_nll_log_scale'),
                'beta_a_raw': tf.Variable(0.5, trainable=True, name=f'{_t}_beta_a_raw'),
                'beta_b_raw': tf.Variable(0.5, trainable=True, name=f'{_t}_beta_b_raw'),
                'gamma_a_raw': tf.Variable(0.5, trainable=True, name=f'{_t}_gamma_a_raw'),
                'gamma_log_scale': tf.Variable(0.0, trainable=True, name=f'{_t}_gamma_log_scale'),
            }
...
#         self.loss_weights = tf.Variable([1.0, 1.0], trainable=True, name='grad_norm_loss_weights')
    ...
    def compute_local_losses(self, predictions, labels):
        """Prediction loss（可选残差分布假设）

        默认使用 weighted BCE（Bernoulli 残差假设）。
        也支持把 residual = y - sigmoid(logit) 当作连续变量，使用不同分布的 NLL：
        - normal: Gaussian NLL（scale 可训练）
        - laplace: Laplace NLL（scale 可训练）
        - t: Student-t NLL（df 固定为 self.t_df，scale 可训练）
        - beta: 把 residual 映射到 (0,1) 后做 Beta NLL（a,b 可训练）
        - gamma: 对 |residual| 做 Gamma NLL（shape/scale 可训练）

        注意：对极不平衡的二分类，依然保留 pos_weight（仅对 y==1 的样本放大权重）。
        """
        ...
        for target_name in self.targets:
            ...
            logit = tf.cast(predictions[target_name], tf.float32)
            label = tf.cast(labels[target_name], tf.float32)

            # 正样本加权（保持与原 BCE 的不平衡处理一致）
            sample_weights = tf.where(label > 0.5, tf.cast(pos_weight, tf.float32), 1.0)

            dist = (self.pred_loss_dist or 'bernoulli').lower()

            if dist in ['bernoulli', 'bce', 'weighted_bce']:
                # 标准 Weighted BCE（对 logits 做数值裁剪）
                logit_clip = tf.clip_by_value(logit, -10.0, 10.0)
                loss_per_sample = tf.nn.weighted_cross_entropy_with_logits(
                    labels=label,
                    logits=logit_clip,
                ...
                # 概率空间 residual
                logit_clip = tf.clip_by_value(logit, -10.0, 10.0)
                prob = tf.math.sigmoid(logit_clip)
                residual = label - prob  # [-1, 1]

                # 取出该 target 的可训练参数
                p = self._nll_params.get(target_name, {})
                scale = tf.nn.softplus(p.get('log_scale', 0.0)) + 1e-3  # >0

                if dist in ['normal', 'gaussian']:
                    # NLL = 0.5*log(2πσ^2) + 0.5*(r/σ)^2
                    standardized = residual / scale
                    loss_per_sample = 0.5 * tf.math.log(2.0 * np.pi) + tf.math.log(scale) + 0.5 * tf.square(standardized)

                elif dist in ['laplace', 'double_exponential']:
                    # NLL = log(2b) + |r|/b
                    loss_per_sample = tf.math.log(2.0 * scale) + tf.abs(residual) / scale

                elif dist in ['t', 'student_t', 'student']:
                    # Student-t NLL（df 固定）
                    df = tf.cast(self.t_df, tf.float32)
                    standardized = residual / scale
                    # logpdf: const + log(scale) + ((df+1)/2)*log(1 + z^2/df)
                    const = tf.math.lgamma((df + 1.0) / 2.0) - tf.math.lgamma(df / 2.0) - 0.5 * tf.math.log(df * np.pi)
                    loss_per_sample = -(const - tf.math.log(scale) - ((df + 1.0) / 2.0) * tf.math.log(1.0 + tf.square(standardized) / df))

                elif dist in ['beta']:
                    # 把 residual 映射到 (0,1)：u = (r+1)/2
                    u = (residual + 1.0) * 0.5
                    u = tf.clip_by_value(u, eps, 1.0 - eps)

                    a = tf.nn.softplus(p.get('beta_a_raw', 0.5)) + 1.1
                    b = tf.nn.softplus(p.get('beta_b_raw', 0.5)) + 1.1

                    # -log Beta pdf
                    log_norm = tf.math.lgamma(a) + tf.math.lgamma(b) - tf.math.lgamma(a + b)
                    loss_per_sample = -( (a - 1.0) * tf.math.log(u) + (b - 1.0) * tf.math.log(1.0 - u) - log_norm )

                elif dist in ['gamma']:
                    # 对 |residual| 做 Gamma（正偏）: x>0
                    x = tf.abs(residual) + eps
                    a = tf.nn.softplus(p.get('gamma_a_raw', 0.5)) + 1.1
                    g_scale = tf.nn.softplus(p.get('gamma_log_scale', 0.0)) + 1e-3
                    # -logpdf(x; a, scale)
                    loss_per_sample = -( (a - 1.0) * tf.math.log(x) - x / g_scale - a * tf.math.log(g_scale) - tf.math.lgamma(a) )

                else:
                    # 不认识的分布 -> 回退到 bernoulli
                    logit_clip = tf.clip_by_value(logit, -10.0, 10.0)
                    loss_per_sample = tf.nn.weighted_cross_entropy_with_logits(
                        labels=label,
                        logits=logit_clip,
