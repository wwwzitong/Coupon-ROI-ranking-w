...
class SEBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.squeeze = tf.keras.layers.Dense(max(1, input_dim // self.ratio), activation='relu', kernel_initializer='glorot_normal')
        self.excitation = tf.keras.layers.Dense(input_dim, activation='sigmoid', kernel_initializer='glorot_normal')
        super(SEBlock, self).build(input_shape)

    def call(self, inputs):
        z = self.squeeze(inputs)
        w = self.excitation(z)
        return inputs * w

class SEBlock_base_DFCL(tf.keras.Model): # std+2pos
    """
...
#         self.ratios = [0.1, 0.5, 1.0]  #先用少量测试
        ...
        for target in self.targets:
            ...
            self.task_towers[name] = tf.keras.Sequential([
                # 第一层：129 -> 64
                tf.keras.layers.Dense(tower_dims[0], activation='relu', kernel_initializer='glorot_normal'),
                # 在激活后加入 SEBlock，对 64 个特征进行重加权
                SEBlock(ratio=4), 
                tf.keras.layers.Dropout(0.1),
                
                # 第二层：64 -> 32
                tf.keras.layers.Dense(tower_dims[1], activation='relu', kernel_initializer='glorot_normal'),
                # 对 32 个特征进行重加权
                SEBlock(ratio=2), # 维度变小了，ratio 也可以调小一点
