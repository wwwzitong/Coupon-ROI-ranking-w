...
#         self.loss_weights = tf.Variable([1.0, 1.0], trainable=True, name='grad_norm_loss_weights')
    ...
    def call(self, inputs, training=True): # 定义数据从输入到输出的完整流动路径       
        ...
        for feature_name in self.dense_feature_names:
            fcd = inputs[feature_name] # 获取当前f_i的所有用户的数据
            fcd = tf.math.log1p(tf.maximum(fcd,0.0)) # 平滑对数变换log(1+x)，处理长尾分布，保证数值稳定性，清洗掉非法负值噪声
            fcd = (fcd - tf.reduce_mean(fcd)) / (tf.math.reduce_std(fcd) + 1e-8) # In-Batch Normalization
            user_dense_vectors.append(tf.reshape(fcd, [-1, self.dense_feature_dim])) # 转变成列向量: (batch_size, 1)

            # 保存每个 dense feature 的 fcd（用于TensorBoard）
            if training:
        ...
        dense_tower_input = tf.concat(user_dense_vectors, axis=1) # 将所有特征拼接: (batch_size, 12)
        ...
        for treatment_id, treatment_features in fixed_treatment_vec_blocks.items():
            ...
            shared_output = tf.concat([dense_tower_output, treatment_vectors_shortcut], axis=1) # 融合user feature和treatment feature
    ...
    def compute_local_losses(self, predictions, labels): #和论文不一致啊
        ...
        for target_name in self.targets:
            ...
            out = tf.minimum(logit, 10.0) # 截断，防止exp爆炸
            term1 = -label * out
            term2 = (1 + label) * (tf.maximum(out, 0) + tf.math.log(1 + tf.exp(-tf.abs(out)))) # 公司常用的loss方法，尝试MSE但效果并不好
    ...
    def decision_policy_learning_loss(self, predictions, labels): #decision loss也和论文有出入
        ...
        weight_tensor = tf.reshape(weight_tensor, [-1, 1]) # 将权重向量形状调整为(batch_size, 1)
        for ratio in self.ratios:
            # $Profit = GMV - lambda x Cost$
            # 让模型学习在不同 $\lambda$ 下都能做出最优决策
            # 使用列表推导，避免显式 append
            values = [
                pred_dict[f"paid_treatment_{t}"] - ratio * pred_dict[f"cost_treatment_{t}"] # u_i = pred_paid_i - lambda x pred_cost_i
                for t in self.treatment_order
            ]
            cancat_tensor = tf.nn.softmax(tf.stack(values, axis=1), axis=1) # pi(t|x)

            # mask_tensor 是 one-hot 编码
            # 确保了 Loss 函数只优化模型对真实发生路径的概率估计与真实收益之间的关系
            ...
            # cancat_tensor * mask_tensor 的结果是，只保留模型对真实 treatment 的“最优概率”预测，其他位置为0，即取出模型分配给真实干预的概率 $\pi(t_{actual} | x)$
    ...
    def train_step(self, data):
        ...
        # 把每个feature的 fcd 分布写进 TensorBoard
